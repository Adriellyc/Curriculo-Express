...
router.post("/", async (req, res) => {
  const { first_name, last_name, title, summary, email, phone, location } =
    req.body;
  try {
    const result = await db.query(
      `INSERT INTO people (first_name, last_name, title, summary, email, phone, location)
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const result = await db.query(
      `UPDATE people SET first_name=$1,last_name=$2,title=$3,summary=$4,email=$5,phone=$6,location=$7
